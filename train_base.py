"""Train the vanilla classifier based on transformer encoder"""
...
from models import VanillaTransformerEncoderClassifier
...
parser.add_argument("--resume", help="Resume training using the saved model", action="store_true")
args = parser.parse_args()
if args.smoke_test:
    epochs, limit_train_batches, limit_val_batches, log_every_n_steps = 2, 20, 10, 2
else:
    epochs, limit_train_batches, limit_val_batches, log_every_n_steps = 100, 1.0, 1.0, 50

L.seed_everything(args.seed, workers=True)
torch.set_float32_matmul_precision("high")
model_checkpoint = ModelCheckpoint(
    monitor="val_f1", mode="max", verbose=False, filename="{epoch}_{val_f1:.4f}", save_last=True
)
...
model = VanillaTransformerEncoderClassifier(num_celltypes=num_classes, label_encoder=dm.label_encoder)
if args.saved_dir is None:  # train the model from scratch and test its performance
    trainer.fit(model=model, datamodule=dm)
    trainer.test(ckpt_path="best", datamodule=dm)
else:
    if args.resume:  # use the saved checkpoint to resume training and test its performance
        trainer.fit(model=model, datamodule=dm, ckpt_path=args.saved_dir)
        trainer.test(ckpt_path="best", datamodule=dm)
    else:  # just test its performance or speed
        if args.stream:
